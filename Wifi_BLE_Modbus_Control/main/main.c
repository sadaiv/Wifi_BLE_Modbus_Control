...
static int  ble_svc_gatt_handler(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        // Copy from BLE stack mbuf to buffer
        int rc = ble_hs_mbuf_to_flat(ctxt->om, buf, len, NULL);
        if (rc == 0) {
            buf[len] = '\0'; // Null terminate for safe string print

            // Log as string
            MODLOG_DFLT(INFO, "Received (%d bytes): %s\n", len, buf);
            xQueueSend(bleCommandQue, (void*)buf, (TickType_t)10);
            // Log as hex if binary
            MODLOG_DFLT(INFO, "BLE command: %s", buf);
            // for (int i = 0; i < len; i++) {
            //     MODLOG_DFLT(INFO, "%02X ", buf[i]);
            // }
            MODLOG_DFLT(INFO, "\n");
        }
        break;
        ...
        if (xQueueReceive(spp_common_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY))            {
            ...
            case UART_DATA:
                if (event.size) {
                    ...
                    MODLOG_DFLT(INFO,"UART Received Data = %s\n", ntf);
            ...
            MODLOG_DFLT(INFO, "UART Event Received");
                break;
            default:
                break;
            }
...
void app_main(void)
{
    ...
    gpio_set_level(RELAY_2, 0); // Set to HIGH
    gpio_set_level(RELAY_3, 0); // Set to HIGH
    gpio_set_level(RELAY_4, 0); // Set to HIGH
